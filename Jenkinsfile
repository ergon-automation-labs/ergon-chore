    ...
    stage('Publish Deploy Event') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/chore_bot/bin/chore_bot"
...
EOF
            ...
            /opt/bot_army/scripts/nats_publish.sh ops.deploy.chore_bot "$PAYLOAD" || echo "⚠️  NATS publish failed (non-blocking)"
            exit 1
          fi
...
EOF
          ...
          $RELEASE_BIN eval 'ChoreBot.Release.migrate()' && {
            echo "✓ Migrations complete"
            echo "Publishing deploy_complete event..."
            VERSION=$(awk '{print $2}' "$RELEASE_DIR/current/chore_bot/releases/start_erl.data" 2>/dev/null || echo "unknown")
            PAYLOAD=$(cat <<EOF
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          echo "Running: $RELEASE_BIN eval 'ChoreBot.Release.migrate()'"
