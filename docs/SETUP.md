# Getting Started with bot_army_chore

This guide walks you through setting up the Chore Bot for local development.
...
releases: [
  chore_bot: [
    applications: [bot_army_chore: :permanent]
  ]
]
```

This creates a release named `chore_bot` that is deployed to `/opt/ergon/releases/chore_bot/` on the server.
...
DATABASE_NAME=bot_army_chore_dev
```
...
3. Extracts and deploys to `/opt/ergon/releases/chore_bot/`
